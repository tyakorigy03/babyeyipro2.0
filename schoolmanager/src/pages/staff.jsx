        ...
        <aside className="w-56 border-r border-slate-100 p-3 bg-white">
          ...
          <nav className="space-y-0.5">
            {departments.map((dept) => (
              <div 
                key={dept.name}
                className={`flex items-center justify-between px-2 py-1 rounded cursor-pointer text-xs transition-all ${dept.name === 'All' ? 'bg-accent/5 text-accent font-semibold' : 'text-slate-600 hover:bg-slate-50'}`}
              >
                <span>{dept.name}</span>
                <span className={`text-[10px] font-medium ${dept.name === 'All' ? 'text-accent' : 'text-slate-400'}`}>{dept.count}</span>
              </div>
            ))}
          </nav>
        </aside>
          ...
          <table className="w-full text-left border-collapse table-fixed">
            ...
            <tbody className="text-[11px]">
              {employees.map((emp, index) => (
                <tr key={emp.id} className={`border-b border-primary/5 cursor-pointer transition-colors hover:bg-primary/5 ${index % 2 !== 0 ? 'bg-primary/5' : ''}`}>
