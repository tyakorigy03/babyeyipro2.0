        ...
        {stages.map(stage => (
          <div key={stage} className="flex-shrink-0 w-80 flex flex-col h-full">
            <div className="flex items-center justify-between mb-4 px-2">
              ...
              <Plus size={14} className="text-slate-400 cursor-pointer hover:text-primary transition-colors" />
            </div>
            ...
            <div className="flex-1 space-y-3 overflow-y-auto pr-2 custom-scrollbar">
              {sampleEnrollmentData.filter(d => d.stage === stage).map(student => (
                <motion.div 
                  ...
                  className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:shadow-md hover:border-primary/20 transition-all group cursor-pointer relative overflow-hidden"
                >
                   <div className="absolute top-0 left-0 w-1 h-full bg-primary/10 group-hover:bg-primary transition-colors" />
                   
                   <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-primary/5 flex items-center justify-center text-[10px] font-bold text-primary border border-primary/10 uppercase">
                          {student.name.split(' ').map(n => n[0]).join('')}
                        </div>
                        <div>
                          <h4 className="text-xs font-bold text-slate-800 group-hover:text-primary transition-colors">{student.name}</h4>
                   ...
                   <div className="space-y-2">
                      ...
                      <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                        <motion.div 
                          ...
                          className={`h-full ${student.stage === 'Review' ? 'bg-accent' : 'bg-primary'}`}
                        />
                      </div>
                   </div>
                   ...
                   <div className="mt-4 pt-3 border-t border-slate-50 flex items-center justify-between">
                      ...
                      <button className={`px-3 py-1 rounded text-[10px] font-black uppercase tracking-widest transition-all ${
                        student.stage === 'Draft' 
                          ...
                          : 'bg-accent/5 text-amber-600 hover:bg-accent hover:text-white'
                      }`}>
