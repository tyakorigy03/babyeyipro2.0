...
export default function Timetable() {
    ...
    const getSubjectColorTheme = (subjectName) => {
        if (!subjectName) return 'bg-white border-slate-100 text-primary';
        const themes = [
        ...
        <div className="flex flex-col h-full bg-white border border-slate-200 overflow-hidden font-sans">
            ...
            <div className="flex items-center justify-between px-5 py-2 border-b border-slate-200 bg-white sticky top-0 z-20 shrink-0">
                ...
                <div className="flex items-center gap-5">
                    {/* Search Field */}
                    <div className="flex items-center border border-slate-300 rounded-md overflow-hidden focus-within:border-accent bg-white">
                    ...
                    <div className="flex items-center border border-slate-300 rounded-md overflow-hidden bg-white shadow-sm font-bold text-[10px] uppercase tracking-widest">
                        <button 
                            ...
                            className={`px-4 py-2 transition-all ${viewMode === 'Draft' ? 'bg-accent/10 text-accent border-r border-slate-300' : 'text-slate-400 hover:bg-slate-50 border-r border-slate-300'}`}
                        >
                            Draft
                        </button>
                        <button 
                            onClick={() => setViewMode('Review')}
                            className={`px-4 py-2 transition-all ${viewMode === 'Review' ? 'bg-accent/10 text-accent' : 'text-slate-400 hover:bg-slate-50'}`}
                        >
                            Review
                        </button>
                    </div>
