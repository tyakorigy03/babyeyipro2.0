    ...
    <div className="flex flex-col h-full bg-white border border-slate-200 overflow-hidden text-sm text-slate-700">
      ...
      <div className="flex items-center justify-between px-5 py-2 border-b border-slate-200 bg-white relative">
        <div className="flex items-center gap-3">
          <Link to="/students/parents/new" className="bg-primary hover:bg-primary/90 text-white px-4 py-1.5 rounded-md text-sm font-semibold tracking-wide shadow-sm transition no-underline">
            New Parent
          </Link>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-56 border-r border-slate-100 p-3 bg-white overflow-y-auto">
          <h2 className="text-[10px] font-bold text-slate-800 uppercase tracking-wider mb-3 flex items-center gap-2">
            <Heart className="w-3 h-3 text-primary" /> RELATIONSHIPS
          </h2>
          ...
          <nav className="space-y-0.5">
            {filterGroups.map((group, idx) => (
              <div 
                key={idx}
                className={`flex items-center justify-between px-2 py-1.5 rounded cursor-pointer text-xs transition-all ${group.name === 'All Parents' ? 'bg-accent/5 text-accent font-semibold' : 'text-slate-600 hover:bg-slate-50'}`}
              >
                <span>{group.name}</span>
                <span className={`text-[10px] font-medium ${group.name === 'All Parents' ? 'text-accent' : 'text-slate-400'}`}>{group.count}</span>
              </div>
            ))}
          </nav>
          ...
          <div className="mt-8 p-3 rounded bg-slate-50 border border-slate-100">
            ...
            <button className="w-full flex items-center justify-center gap-2 p-2 bg-white border border-slate-200 rounded hover:border-primary/30 transition-all text-[9px] font-bold text-slate-500 uppercase">
          ...
          <table className="w-full text-left border-collapse table-fixed">
            ...
            <tbody className="text-[11px]">
              {parentsData.map((parent, index) => (
                <tr key={parent.id} className={`border-b border-primary/5 cursor-pointer transition-colors hover:bg-primary/5 ${index % 2 !== 0 ? 'bg-primary/5' : ''}`}>
                  <td className="px-4 py-2 w-10"><input type="checkbox" className="rounded border-slate-300 text-primary focus:ring-primary/20" /></td>
