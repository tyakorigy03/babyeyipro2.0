...
const authFetch = async (url, options = {}) => {
  const token = localStorage.getItem('babyeyipro_auth_token');
  const headers = {
...
const foundationService = {
  ...
  try {
      const response = await authFetch('/transportRoutes');
      return response.data || [];
    } catch (error) {
      console.error('Failed to fetch transportRoutes:', error);
      return [];
    }
  }
};
