...
function buildInitialTeachers() {
  const teachers = [
    { id: 1, name: 'Robert Ntaganda', dept: 'ICT Department' },
    { id: 2, name: 'David Kagabo',    dept: 'ICT Department' },
    { id: 3, name: 'Diane Umutoni',   dept: 'Literature Department' },
    { id: 4, name: 'Samuel Bizimana', dept: 'Literature Department' },
    { id: 5, name: 'Patrick Nshimiyimana', dept: 'Science Department' },
    { id: 6, name: 'Jean Baptiste',   dept: 'Mathematics Department' },
  ];
  
  return teachers.map(teacher => ({
    ...teacher,
    attendance: WEEK_DAYS.map(() => ALL_PERIODS.map(p => p.type === 'p' ? 'na' : 'skip')),
  }));
}
...
export default function TeacherAttendance() {
  ...
  const [teachers, setTeachers] = useState(buildInitialTeachers);

  const toggle = (teacherId, dayIdx, periodIdx) => {
    setTeachers(prev => prev.map(t => {
      if (t.id !== teacherId) return t;
      const att = t.attendance.map(day => [...day]);
      const cur = att[dayIdx][periodIdx];
      att[dayIdx][periodIdx] = cur === 'na' ? 'present' : cur === 'present' ? 'absent' : 'na';
      return { ...t, attendance: att };
    }));
  };

  const filteredTeachers = teachers.filter(t =>
    t.name.toLowerCase().includes(searchQuery.toLowerCase()) &&
    (selectedDept === '' || t.dept === selectedDept)
  );

  // Stats for selected day
  const todayIdx = Math.min(Math.max((new Date().getDay() - 1), 0), 4);
  const presentCount = teachers.reduce((acc, t) => acc + t.attendance[todayIdx].filter(v => v === 'present').length, 0);
  const absentCount = teachers.reduce((acc, t) => acc + t.attendance[todayIdx].filter(v => v === 'absent').length, 0);
    ...
    <div className="flex flex-col h-full bg-white border border-slate-200 overflow-hidden font-sans">
      ...
      <div className="flex items-center justify-between px-5 border-b border-slate-200 bg-white sticky top-0 z-20 shrink-0 h-14">
        <div className="flex items-center gap-6">
          ...
          <h1 className="text-sm font-semibold text-slate-800">Teacher Attendance</h1>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* ── Sidebar ── */}
        <aside className="w-56 border-r border-slate-100 p-3 bg-white overflow-y-auto shrink-0 custom-scrollbar">
          <div className="mb-4">
          ...
          <h2 className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1.5 mb-2">
            <Users size={9} className="text-primary" /> Departments
          </h2>
          <nav>
            <div
              onClick={() => setSelectedDept('')}
              className={`px-2 py-1.5 rounded cursor-pointer text-[10px] font-bold uppercase tracking-tight transition-all mb-1 ${selectedDept === '' ? 'bg-primary/5 text-primary font-black border border-primary/20' : 'text-slate-600 hover:bg-slate-50 border border-transparent'}`}
            >
              All Teachers
            </div>
        ...
        {/* ── Main Content ── */}
        <main className="flex-1 overflow-auto bg-slate-50/50 flex flex-col custom-scrollbar">
            ...
            {filteredTeachers.length === 0 ? (
              ...
              >
                ...
                <p className="text-xs text-slate-500 mt-1 max-w-xs">
                  {selectedDept ? `No teachers assigned to ${selectedDept}.` : 'No teachers match the search query.'}
                </p>
              </motion.div>
            ) : (
              <motion.div
                ...
                <div className="bg-white border border-slate-200 overflow-hidden flex flex-col h-full">
                  <div className="overflow-auto flex-1">
                    ...
                    >
                      <thead className="sticky top-0 z-10 bg-white">
                        ...
                        <tr className="border-b border-slate-200">
                          ...
                          {WEEK_DAYS.map((day, dIdx) => (
                            <th key={dIdx} colSpan={ALL_PERIODS.length} className="border-r border-slate-300 text-center text-[9px] font-black uppercase tracking-widest text-slate-700 bg-slate-50 py-1.5">{day.label}</th>
                          ))}
                        </tr>
                        <tr className="border-b border-slate-200 bg-slate-50">
                          {WEEK_DAYS.map((_, dIdx) =>
                            ALL_PERIODS.map((p, pIdx) => (
                              <th key={`${dIdx}-${pIdx}`} className={`text-center py-1 border-r text-[7.5px] font-black uppercase tracking-tight leading-tight ${p.type === 'break' || p.type === 'lunch' ? 'border-slate-200 text-slate-400 bg-slate-100/80' : 'border-slate-200 text-slate-500'} ${pIdx === ALL_PERIODS.length - 1 ? 'border-r-slate-300' : ''}`} style={{ width: 28, minWidth: 28 }}>
                                <div>{p.label}</div>
                      ...
                      <tbody>
                        {filteredTeachers.map((t, rowIdx) => (
                          <tr key={t.id} className={`border-b border-slate-100 hover:bg-primary/[0.02] transition-colors ${rowIdx % 2 !== 0 ? 'bg-slate-50/60' : 'bg-white'}`}>
                            <td className="border-r border-slate-200 text-center text-[8px] text-slate-400 font-bold" style={{ width: 28 }}>{rowIdx + 1}</td>
                            <td className="px-2 border-r border-slate-200" style={{ width: 140 }}>
                              <div className="flex items-center gap-2">
                                <div className="w-5 h-5 rounded-full bg-primary/10 flex items-center justify-center text-[8px] font-bold text-primary shrink-0 border border-primary/15 uppercase">{t.name.charAt(0)}</div>
                                <div className="min-w-0">
                                  <p className="text-[9px] font-bold text-slate-800 truncate leading-tight">{t.name}</p>
                                  <p className="text-[7.5px] font-medium text-slate-400 uppercase tracking-tighter truncate leading-tight">{t.dept}</p>
                                </div>
                              </div>
                            </td>
                            {WEEK_DAYS.map((_, dIdx) =>
                              ALL_PERIODS.map((p, pIdx) => (
                                <AttCell key={`${dIdx}-${pIdx}`} status={t.attendance[dIdx][pIdx]} onClick={p.type === 'p' ? () => toggle(t.id, dIdx, pIdx) : undefined} />
                              ))
                            )}
                          </tr>
                        ))}
